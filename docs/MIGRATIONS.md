...
| `20260628095313_seed_base_prices.sql` | `price_list` (seed) | _pending_ | Hardik | M18/M19. Seeds base prices from `skus.rate_per_case` (37/46 SKUs; selling rate from workbook). Idempotent (`not exists` guard); unpriced SKUs left for client to confirm. |
