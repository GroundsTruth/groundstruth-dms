...
| `20260628100608_next_invoice_no_fn.sql` | `next_invoice_no()` fn | _pending_ | Hardik | M20. Atomic invoice numbering: reads+increments `config.invoice_series` under `for update` lock (no dup/skip), self-heals missing row. `execute` to `service_role` only. |
