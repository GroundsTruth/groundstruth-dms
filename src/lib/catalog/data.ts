...
/**
 * Canonical SKU list, cleaned from the Jaypee `seed_skus.csv` (52 rows):
 ...
 * Deeper de-duplication (e.g. the two "CSD Cola 200 ML" rows) is left for the
 * operator to confirm — we flag, we don't silently guess.
 *
 ...
 *
 * NOTE: this static seed is swapped for a Supabase server query later — keep
 * `getSkus()` as the single accessor so callers don't change.
 */
...
/**
 * Single accessor for the SKU list. Async on purpose: today it returns the
 * static seed; later it becomes a Supabase server query with the same signature.
 */
